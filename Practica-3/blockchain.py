...
def listing():
    response = {
            'message':'List of all the registred nodes',
...
def validate():
    if blockchain.valid_chain(blockchain.chain):
        response = {'mesage':'Valid blockchain'}
    else:
        response = {'mesage':'Invalid blockchain'}
...
def manipulate():
    last_block = blockchain.last_block
    proof = blockchain.proof_of_work(last_block)
    blockchain.modificar(proof)
    
    response = {'mesage':'blockchain modified'}
    return jsonify(response),200
